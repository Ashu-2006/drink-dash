/* ==========================================================================
   Flood type: engine. Supplied implementation, unmodified.
   ========================================================================== */
...
/** Heavy tailed, not uniform: most letters barely move and one or two go wild.
    A uniform draw over the same range triples the median and the whole thing
    collapses into even noise. */
...
/** Tracking has to be added to the measured line width before solving the font
    size, or every lockup comes out narrower than its target by the sum of its
    gaps. n glyphs have n-1 gaps. */
function lineWidth(
...
export class FloodType {
  ...
  /** Two constraints, take the smaller: solve so the longer line hits a target
      fraction of the card width, then clamp so the cap height cannot exceed a
      fraction of the card height. Both bind in practice on different words. */
  private layout() {
    const ctx = this.ctx;
    ...
    const capProbe = ctx.measureText("H").actualBoundingBoxAscent || PROBE * 0.7;
    ...
    this.c1 =
      b1 + (m1.actualBoundingBoxDescent - m1.actualBoundingBoxAscent) / 2 - mid;
    this.c2 =
      b2 + (m2.actualBoundingBoxDescent - m2.actualBoundingBoxAscent) / 2 - mid;
    ...
    for (const [line, baseline, li] of lines) {
      ...
      for (const ch of line) {
        ...
        // Pivot on the ink box, not the advance box. The two differ by several
        // percent of the em and a letter turned about its advance centre
        // visibly swims sideways as it spins.
    ...
    // Order the entrance by distance from the centre, not by index: outermost
    // letters land first, the middle lands last, so the word closes on its own
    // middle rather than reading as a left to right wave.
    let maxDist = 0;
  ...
  start() {
    ...
    const loop = (now: number) => {
      ...
      // The pointer owns the hold, not the flood. Both bounds are needed: a
      // gate of merely "past the end of the hold" stays true for the rest of
      // the cycle and would freeze the type mid flood.
  ...
  /** The still frame is the settled lockup, not frame zero: with an entrance in
      front, frame zero is an empty card with the letters above the top edge. */
  renderStill() {
  ...
  private draw() {
    ...
    // The lean must fade to zero as the zoom takes over, or it carries the word
    // off centre exactly when it should be arriving dead centre.
    ...
    for (const g of this.glyphs) {
      ...
      // The turn lags the drop, or the whole effect happens above the top edge
      // where nobody sees it.
      const turned = Math.pow(Math.max(arrive, 0), TURN_LAG);
      const yaw = g.turnIn * (1 - turned) + g.tumble * drop;
      // One cosine is the entire 3D model: a glyph drawn at a horizontal scale
      // of cos(yaw) is that glyph turned about its own vertical axis under
      // orthographic projection. Keep the sign: negative is the back face.
      ...
      // Depth scales the letter's SIZE only. Its POSITION uses the shared
      // global scale, because glyph offsets are symmetric about the line centre
      // and multiplying each by its own depth would drift the word off centre.
      const depthNow = DEPTH_AT_REST + (1 - DEPTH_AT_REST) * zoom;
      const sc = scale * (1 + g.depth * DEPTH_SPREAD * depthNow);

      const par = 1 + g.depth * 0.6;
      const gx = cx + (g.x + g.px) * scale * squeeze + dx + focusX + leanX * par;
    ...
    // Sort by depth and draw back to front, so nearer letters paint over
    // further ones. Nearly sorted frame to frame, so insertion beats a
    // comparator sort that allocates every frame.
    for (let i = 1; i < n; i++) {
      const s0 = draws[i];
    ...
    // Smear the fast letters rather than blurring them: repeated flat stamps
    // read as speed the way a printed multiple exposure does, and cost far
    // less. Convert the per frame delta using this frame's own duration, or a
    // fixed threshold fires on a 30Hz display and never on a 120Hz one.
