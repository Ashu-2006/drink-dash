/* ==========================================================================
   Ink flood: procedural scribble. Supplied implementation, unmodified.

   The trap a plain cosine falls into: evenly spaced humps of equal height read
   as a W or a graph, never as handwriting. Humps are built one at a time, each
   with its own height and its own share of the width, each with its peak skewed
   off centre so the up stroke and down stroke have different slopes.

   Then width follows speed, INVERTED. A real pen lays more ink where the hand
   slows, and the valleys of a cursive stroke are both the slowest and the
   fattest point, so deriving radius from local speed is most of the hand in one
   rule. Seeded hash noise, never Math.random, so a scene renders identically
   everywhere.
   ========================================================================== */
...
export const SCRIBBLE_DEFAULTS: ScribbleSpec = {
  ...
  humps: 3,
  amplitude: 0.19,
  /** Match the reference proportions or the flood stalls mid card at the same
      zoom: 87 percent of the square wide, about 31 mean half width. */
  span: 0.87,
  crestR: 26,
  valleyR: 45,
  drift: 0.35,
...
export function generateSpine(input: Partial<ScribbleSpec> = {}): SpineSample[] {
  ...
  for (let i = 0; i <= n; i++) {
    ...
    // Width follows speed, inverted: fat where the hand slows.
    ...
    // Frames are distributed by eased cumulative arc length, not by index,
    // because the pen accelerates out of valleys and coasts over crests.
    const u = cum[i] / total;
...
export function generateTip(
  ...
  // The true centerline sits 6px BELOW the visible dot: it rides slightly high
  // on its own stroke.
...
export function generateFlood(
  ...
  for (let i = 0; i < n; i++) {
    ...
    // k stays at 1 for the first 60 percent of the flood, then climbs.
