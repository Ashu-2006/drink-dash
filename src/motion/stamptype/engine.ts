/* ==========================================================================
   Stamp type: engine. Supplied implementation, unmodified.
   ========================================================================== */
...
export class StampType {
  ...
  start() {
    ...
    const loop = (now: number) => {
      ...
      this.last = now;
      // 20fps on a 10Hz movement cadence: consecutive frames often repeat
      // exactly, and that stutter is the print-like snap. Do not smooth it.
  ...
  /** Lines longer than the layout expects are condensed horizontally to the
      room their own track leaves them, rather than set smaller, because four
      different type sizes read as a layout instead of as one poster. */
  private measure(
  ...
  /** The rounded bar unioned with the glyphs stroked fat in bar colour. That
      union is where the descender drips along the bottom edge come from. */
  private silhouette(
    ctx: CanvasRenderingContext2D,
    line: Line,
    x: number,
  ...
  /** Stamps are computed fresh each frame from the tracks, so there is no
      accumulation state and the loop seam, resizes and tab resumes all land on
      exactly the right residue with no replay bookkeeping. */
  ...
  /** All four bars first, then all four inks: parked rows sit 68px apart but a
      bar with a descender runs about 75px, so per line bar-then-ink would let
      the next row's bar bite chunks out of the previous row's letterforms. */
  ...
  private draw(tick: number) {
    ...
    // The field swaps under the densest part of the interleaved trails, as a
    // hard cut. Nothing in the piece fades, and a gradient on the field is the
    // one thing that would give away it is not print.
